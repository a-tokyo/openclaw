...
- **`maintenance`**: session-store cleanup + retention controls.
  ...
  - `maxEntries`: maximum total number of live SQLite session entries (default `500`). Every row counts toward the cap. Always-protected rows — archived or pinned sessions, active or admitted work, model-locked sessions, and the primary `main`/`global` session — are never automatic eviction targets, but still consume the cap. Durable conversations (threads, Telegram topics, group/channel roots) are exempt from age pruning (`pruneAfter`) and become evictable oldest-first under `maxEntries` and `maxDiskBytes`. Cleanup removes the oldest eligible rows until it reaches `maxEntries` or runs out of victims. The total can therefore remain above the cap when always-protected rows alone exceed it or active work temporarily blocks eviction. Runtime writes batch cleanup with a small high-water buffer for production-sized caps; `openclaw sessions cleanup --enforce` applies the cap immediately but does not unprotect rows. Unarchive, unpin, wait for active work to finish, raise the caps, or explicitly delete sessions you no longer want to retain.
  - `preserveRecent`: optional inactivity window that protects recently active interactive sessions and all of their SQLite history generations from age pruning (`pruneAfter`) and SQLite historical-generation disk cleanup (for example `"7d"`). It does not block `maxEntries` or live-node `maxDiskBytes` eviction. Unset or `false` disables this protection. Synthetic model-run, cron, hook, heartbeat, ACP, and sub-agent sessions remain eligible for bounded cleanup. Protection does not archive sessions.
